      ...
      <Dialog open={isOpen} onOpenChange={(open) => !open && handleModalClose()}>
        <DialogContent className="sm:max-w-[600px] md:max-w-[800px] lg:max-w-[900px] p-0 max-h-[90vh] flex flex-col bg-card">
           {processingInitialAction && !item ? ( 
            ...
            <>
              <div className="flex-grow overflow-y-auto scrollbar-hide"> {/* This div will scroll */}
                {item.bannerFundo && (
                  <div className="relative h-48 md:h-64 w-full">
                    <Image
                      ...
                      layout="fill"
                      objectFit="cover"
                      className="rounded-t-md" // Note: rounded-t-md might not be visible if modal itself is not rounded on top
                      data-ai-hint="movie scene tv series background"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-card via-card/80 to-transparent" />
                  </div>
                )}
                <div> {/* Wrapper for DialogHeader and content grid */}
