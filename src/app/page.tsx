...
import { HomeAniDetailModal } from '@/components/homeani/HomeAniDetailModal'; // Added import
import { Loader2, AlertTriangle, Film } from 'lucide-react';
import { Button } from '@/components/ui/button';

export default function HomeAniPage() {
  const [selectedItem, setSelectedItem] = useState<StoredCineItem | null>(null); // State for modal
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        {!isLoading && !error && items && items.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
            {items.filter(item => item.status === 'ativo').map((item) => (
              <HomeAniContentCard 
                key={item.id} 
                item={item} 
                onClick={() => handleCardClick(item)} // Added onClick
              />
            ))}
          </div>
        )}
